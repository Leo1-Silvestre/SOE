...
def ca_acquisition(t,signal,fs,f_seq,num_periods):
  ...
  #generate reference signal
    ...
    for index_doppler_bin in range(0,len(doppler_bin_vec)):
      ...
      cost_function.append((abs(scipy.fft.ifft(np.conjugate(scipy.fft.fft(signal))*scipy.fft.fft(doppler_bin_signal*reference))))**2) #adaptar

    ACQ_DATA['cost_function'].append(cost_function)
    temp_array = np.array(cost_function)
    max_val = temp_array.max()#talvez apagar
    Maximus[index].append(max_val)
    index += 1
  ...
  #lens = [len(groups[0]),len(groups[1]),len(groups[2])]
  ...
  #if len(groups[0])==min(lens):
   #  index = [1,2]
    # sats_found = np.where(labels == 0)[0]+1
  #elif len(groups[1])==min(lens) :
  #   index = [0,2]
  #   sats_found = np.where(labels == 1)[0]+1
  #else:
  #   index = [0,1]
  #   sats_found = np.where(labels == 2)[0]+1
  ...
  if len(sats_found)<7:
     ...
     new_sats = heapq.nlargest(6,groups[set_label_2[0][index_label]])


     while(len(sats_found)<6):
        
  #      if len(groups[index[0]])<len(groups[index[1]]):
           
           new_sat = np.where(Maximus==new_sats[j][0])[0][0]+1
           #new_sat = np.where(labels == set_label[0])[0][i]+1
           sats_found = np.append(sats_found,new_sat)
           sats_found.sort()
  #      else:
  #         new_sat = np.where(labels == index[1])[0][i]+1
  #         sats_found = np.append(sats_found,new_sat)
  #         sats_found.sort()
           #i -= 1
           j += 1
...
x = scipy.io.loadmat('x2023_05_24.mat')
x = x['x']
x = x[0]
...
#for i in range(0,33,1):
 # if i in sats_found :
   #   axis_y1.append(1)
    #  axis_y2.append(ACQ_DATA['data'][j][2])
    #  j+=1
  #else:
     # axis_y1.append(0)
     # axis_y2.append(0)
...
#plt.stem(axis_x,axis_y2, bottom = threshold)
#plt.show()
